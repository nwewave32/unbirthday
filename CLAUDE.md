...
### /project:create-task <title>
**Handler**: /.claude/commands/create-task.js
**Args**:
- `title` (required): Task title
- `description` (optional): Task description using `--description="desc"`
- `priority` (optional): Task priority using `--priority=high|medium|low`

**Output**: Markdown task entry appended to Backlog.md + optional GitHub Issue link

**Usage Examples**:
```bash
node .claude/commands/create-task.js "Implement user authentication"
node .claude/commands/create-task.js "Fix login bug" --description="Users can't login with special characters" --priority=high
```

**Features**:
- Automatic Backlog.md creation if not exists
- GitHub issue creation (if gh CLI available)
- Backlog CLI integration (if installed)
- Task ID generation with timestamps

---

### /code:review <path>
**Handler**: /.claude/commands/code-review.js
**Args**:
- `path` (required): File path or PR number/URL

**Output**: JSON list of review comments (line, severity, message, suggested_fix) + lint/test summary

**Usage Examples**:
```bash
node .claude/commands/code-review.js src/App.tsx
node .claude/commands/code-review.js "#123"
node .claude/commands/code-review.js "https://github.com/user/repo/pull/123"
```

**Features**:
- File-level code analysis
- PR-level review support
- Language-specific checks (JS/TS, Python)
- Lint and test integration
- Severity levels: error, warning, info, style
- Automatic issue detection (long lines, console.log, var usage, etc.)

---

### /code:refactor <path>
**Handler**: /.claude/commands/code-refactor.js
**Args**:
- `path` (required): File or folder path
- `style` (optional): Coding style using `--style=standard|modern|typed`

**Output**: Git branch with changes pushed, PR created, DoD validation report

**Usage Examples**:
```bash
node .claude/commands/code-refactor.js src/components/Button.tsx
node .claude/commands/code-refactor.js src/utils --style=modern
```
...
- Modern syntax conversion (var → const/let, function → arrow functions)

**DoD Validation Includes**:
- ✅ Linting passes
